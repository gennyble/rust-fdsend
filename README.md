...
- Creates the necessary message/control message structures that we're going to send. This is the first place `libc` is used and where the file descriptor we're sending is input.
- Waits for `recv` to connect over a Unix Socket and immediately grabs a reference to the raw fd with `UnixStream::as_raw_fd`
...
- Create the necessary structures for receiving the message/control message.
